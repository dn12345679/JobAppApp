...
// Word-style menu bar for the Cover Letter screen — the résumé's menu bar copied
// and adapted: File holds the document actions; Edit holds the letter's
// typographic controls (font size, line spacing, find and replace).
...
export default function CoverLetterMenuBar({
  letter,
  savedAt,
  onNew,
  onMakeCopy,
  ...
  onExportFile: () => void;
  onExportPdf: () => void;
  onSettings: (patch: Partial<CoverLetterSettings>) => void;
  onFindReplace: () => void;
}) {
  ...
  return (
    <div className="relative z-30 flex items-center gap-1 border-b border-slate-800 bg-slate-900/60 px-2 py-1 text-sm">
      ...
      >
        <MenuItem onClick={run(onNew)}>New cover letter</MenuItem>
        <MenuItem onClick={run(onMakeCopy)}>Make a copy</MenuItem>
        <MenuItem onClick={run(onOpenManager)}>View / open previous…</MenuItem>
        <Divider />
        <MenuItem onClick={run(onImport)}>Import cover letter…</MenuItem>
        <MenuItem onClick={run(onExportFile)}>Export cover letter (.jtcl)…</MenuItem>
      ...
      >
        ...
        <Stepper
          label="Line spacing"
          display={`${(s.lineSpacing ?? 0) > 0 ? "+" : ""}${s.lineSpacing ?? 0}`}
          onDec={() => onSettings({ lineSpacing: clamp((s.lineSpacing ?? 0) - 2, 0, 12) })}
          onInc={() => onSettings({ lineSpacing: clamp((s.lineSpacing ?? 0) + 2, 0, 12) })}
          onReset={() => onSettings({ lineSpacing: 0 })}
        />
        <Divider />
        <MenuItem onClick={run(onFindReplace)}>Find and replace…</MenuItem>
      </MenuButton>

      <span className="ml-2 truncate text-slate-400" title={letter.name}>
        {letter.name}
      </span>
